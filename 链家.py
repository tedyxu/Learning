...
def get_single_urls(price_low,price_high):
    ...
    for i in range(0, page_num):
        i += 1
        # mw1是满五年，bt2是板楼，y4是20年以内，a3是70-90平，a4是90-110平，bp是价格下限，ep是价格上限
        url = 'https://sh.lianjia.com/ershoufang/pg' + str(i) + 'mw1bt2y4a3a4bp' + price_low + 'ep' + price_high + '/'
...
def get_single_info(url):
    ...
    #面积、建筑年代、建筑类型
    pattern_area = re.compile('<div class="area"><div class="mainInfo">(.*?)平米</div><div class="subInfo">(.*?)年建/(.*?)</div></div>')
    areas = re.findall(pattern_area, req.text)
    for area in areas:
        data_list.append(area[0])
        data_list.append(area[1])
        data_list.append(area[2])
    #挂牌时间
    ...
    #房屋用途
    pattern_yongtu = re.compile('房屋用途</span>.*?<span>(.*?)</span>',re.S)
    yongtu = re.findall(pattern_yongtu,req.text)
    data_list.append(yongtu[0])
...
def main():
    ...
    #打开csv文件并写入标题行
    with open('链家.csv', 'w', newline='', encoding='utf_8_sig') as f:
        csv_writer = csv.writer(f)
        csv_writer.writerow(['名称','总价（万）','单价（元/平方米）','区','地段','环线','小区名称','面积（平方米）','建筑年代','建筑类型','挂牌时间','上次交易时间','户型','楼层','楼层共计','朝向','结构','装修','梯户比例','电梯','交易权属','房屋用途','链接'])
        f.close()
    #一行行写入文件
    count = 1
    for url in urls:
        ...
        #写入csv文件
        with open('链家.csv', 'a', newline='',encoding='utf_8_sig') as f:
            csv_writer = csv.writer(f)
